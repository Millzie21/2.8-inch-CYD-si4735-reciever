...
LGFX_Sprite spr(&tft);
#define BACKLIGHT_INVERT 1
void tftBacklight(uint16_t v) {
  v = constrain(v, MaxBrightness, MinBrightness);
  uint8_t b;
#if BACKLIGHT_INVERT
  b = map(v, MinBrightness, MaxBrightness, 0, 255);
#else
  b = map(v, MaxBrightness, MinBrightness, 0, 255);
#endif
  tft.setBrightness(b);
}
SI4735 si4735;
