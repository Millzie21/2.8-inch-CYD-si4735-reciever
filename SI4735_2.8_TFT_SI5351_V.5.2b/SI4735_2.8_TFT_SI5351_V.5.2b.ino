...
void setBacklight(uint16_t v) {
  int b = map((int)v, MinBrightness, MaxBrightness, 0, 255);  // 16 = bright … 256 = dim
  if (b < 0)   b = 0;
  if (b > 255) b = 255;
  tft.setBrightness(b);
}
...
void setup() {
  ...
  Serial.println("     SI4735/32 CYD Radio");
  Serial.println("Version 6.1b 10-06-2026");
  ...
  tft.println("SI4735/32 CYD Radio");
  tft.setCursor(7, 70);
  tft.println(" Version 6.1b");
  tft.setCursor(7, 95);
  tft.println(" 10-06-2026");
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setCursor(7, 120);
  tft.println(" Millzies Port.- Github");
...
// ===============================================
  ...
  if (!displayPower) setBacklight(currentBrightness);
  freqstep = 1000;//hz
  previousBFO = -1;
...
void saver() {
  ...
  tft.fillScreen(TFT_BLACK);
  if (displayOff) {
    if (displayPower) digitalWrite(DISPLAY_LED, 1); else setBacklight(currentBrightness);
  }
  if (saverOn) {
    Saver = true;
    SEEK = false;
...
//activity  
  ...
  encoderButton = 0;
  if (displayOff) {
    if (displayPower) digitalWrite(DISPLAY_LED, 0); else setBacklight(currentBrightness);
  }
  returnLayer();
  elapsedSaver = millis();
}
